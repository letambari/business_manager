<?php
	if (isset($_POST['key'])) {
		...
		if ($_POST['key'] == 'getRowData') {
			...
			$sql = $conn->query("SELECT task_desc, project_id, company_id FROM task WHERE task_id='$rowID'");
		...
		if ($_POST['key'] == 'getExistingData') {
			...
			$sql = $conn->query("SELECT task_id, task_desc FROM task LIMIT $start, $limit");
			if ($sql->num_rows > 0) {
				$response = "";
				while($data = $sql->fetch_array()) {
                      
					$response .= '<li class="task">
																	<div class="task-container">
																		<span class="task-action-btn task-check">
																			<span class="action-circle large complete-btn" title="Mark Complete" onclick="viewORedit('.$data["task_id"].')">
																				<i class="material-icons">check</i>
																			</span>
																		</span>
																		<span class="task-label" contenteditable="true">'.$data["task_desc"].'</span>
																		<span class="task-action-btn task-btn-right">
																			<span class="action-circle large" title="Assign" onclick="edit('.$data["task_id"].')">
																				<i class="material-icons">person_add</i>
																			</span>
																			<span class="action-circle large delete-btn" title="Delete Task" onclick="deleteRow('.$data["task_id"].')">
