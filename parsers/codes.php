...
$sql_t = "SELECT * FROM task WHERE company_id = '$log_username' AND user_id = '$log_id'";
$query_t = mysqli_query($db_conx, $sql_t);
$count_task = mysqli_num_rows($query_t);



$sql_project_count = "SELECT * FROM project WHERE company_id = '$log_username' AND user_id = '$log_id'";
...
//this code run to get all the tasks im  the compnay
$sql_count_task = "SELECT * FROM task WHERE company_id = '$log_username' AND user_id = '$log_id'";
...
//<!------------------------------------------------------------------------!>
// this is the code for the employees page
 $sql_count_employee = "SELECT * FROM employees WHERE company_id = '$log_username' AND user_id = '$log_id'";

// $sql_count_employee = "SELECT employees.employee_id, departments.department_id, departments.department_name, departments.company_id, departments.user_id FROM employees INNER JOIN departments ON employees.department_id = departments.department_id ";
...
// $sql_count_departments = "SELECT department_name, department_id FROM departments WHERE company_id = '$log_username' AND user_id = '$log_id' AND department_id = '$e_department'";
...
//   $department_id = $row_departments[0];
//   $department_name = $row_departments[1];
...
//   $user_id = $row_departments['user_id'];
  ...
  $getemp .= '<div class="col-md-4 col-sm-6 col-12 col-lg-4 col-xl-3">
              ...
              <!--  <div class="dropdown profile-action">
                ...
                <h4 class="user-name m-t-10 mb-0 text-ellipsis"><a href="profile?empID='.$employee_id.'">'.$e_firstname.' '.$e_lastname.'</a></h4>
...
//<!------------------------------------------------------------------------!>
// this is the End code for the employees page
...
while ($row_ps = mysqli_fetch_array($query_ps)) {
  ...
  $querythetask = mysqli_query($db_conx, $getthetaskinfo);
    ...
    # code...
  $getit2 = "SELECT * FROM clients WHERE company_id = '$log_username' AND user_id = '$log_id' AND client_id = '$pclient_id'";
...
$geteme = "SELECT * FROM employees WHERE company_id = '$log_username' AND user_id = '$log_id' AND employee_id = '$pleader_id'";
...
while ($kkk = mysqli_fetch_array($inquery)) {
  ...
  $inner_projectleader_id = $kkk['leader_id'];
   ...
   //  $getthetask = "SELECT * FROM task WHERE user_id = '$log_id' AND company_id = '$log_username' AND project_id = '$inner_project_id' AND Status = 'Completed'";
   // $yougetthetask = mysqli_query($db_conx, $getthetask);
  //    $countask1 = mysqli_num_rows($yougetthetask);

  //    $getthetask = "SELECT * FROM task WHERE user_id = '$log_id' AND company_id = '$log_username' AND project_id = '$inner_project_id' AND Status = 'Pending'";
   // $yougetthetask = mysqli_query($db_conx, $getthetask);
...
$sql_p = "SELECT * FROM task WHERE company_id = '$log_username' AND user_id = '$log_id'";
$query_p = mysqli_query($db_conx, $sql_p);
while ($row_p = mysqli_fetch_array($query_p)) {
  # code...
  $task_id = $row_p['task_id'];
...
$_SESSION['pidss'] = $pidss;

    $sql_p = "SELECT employees.employee_id, employees.e_firstname, employees.e_lastname, project.leader_id, project.end_date FROM project INNER JOIN employees ON project.leader_id = employees.employee_id WHERE project.project_id = '$pid'";
                                            ...
                                            <div class="message-area">
                                                <div class="input-group">
                                                    <textarea class="form-control" id="msg" name="msg" placeholder="Type message..."></textarea>
                                                     <input type="hidden" id="name" name="name" placeholder="enter name" value="'.$ContactPerson.'" /> 
                                                     <input type="hidden" id="pid" name="pid" placeholder="enter name" value="'.$pid.'" /> 
                                                     <input type="hidden" id="company_id" name="company_id" placeholder="enter name" value="'.$log_username.'" /> 
...
$showfollowers = '';
 ...
 if ($countfollowers > 0) {
    ...
    while ($rowfollowers = mysqli_fetch_array($getfollowers)) {
       ...
       $showfollowers .= '
<a class="avatar" href="#" data-toggle="tooltip" title="'.$pem_firstname.' '.$pem_lastname.'">
                                            <img alt="" src="assets/img/profiles/avatar-16.jpg">
                                        </a>';
    } 
}else {
...
$contacts = '';
  $getemployees = "SELECT * FROM employees WHERE company_id = '$log_username' AND user_id = '$log_id'";
    ...
    $contacts .= '<li>
                <a href="chats?currentchat='.$chat_employee_id.'">
                  <span class="chat-avatar-sm user-img">
                    <img class="rounded-circle" alt="" src="assets/img/profiles/avatar-09.jpg">'.$thestate.'
                  </span> 
                  <span class="chat-user">'.$thefullname.'</span><!-- <span class="badge badge-pill bg-danger">7 --></span>
                </a>
              </li>'; 
  }
...
if (isset($_GET['currentchat'])) {
  ...
  $_SESSION['currentchat'] = $currentchat;
  //exit();
  $getchat = "SELECT * FROM employees WHERE company_id = '$log_username' AND user_id = '$log_id' AND e_username = '$currentchat'";
  ...
  $withoutcontact = '<div class="user-details mr-auto">
                      ...
                      <div class="user-info float-left">
                        <a href="profile.html" title="Mike Litorus"><span>Chosse a contact from the sidebar to chat with</span> <i class="typing-text"><!-- Typing... --></i></a>
                        <span class="last-seen"><!-- Last seen today at 7:50 AM --></span>
                      </div>
                    </div>';
}

$messagebox = ' <div id="upload_form" enctype="multipart/form-data" method="post">
                    ...
                    <div class="message-inner">
                      ...
                      <div class="message-area">
                        <div class="input-group">
                          ...
                          <input type="hidden" id="sender_id" value="'.$log_username.'">
                          <input type="hidden" id="reciever_id" value="'.$_SESSION['currentchat'].'">
                          <input type="hidden" id="sender_fullname" value="'.$ContactPerson.'">
