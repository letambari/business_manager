...
while ($row_p = mysqli_fetch_array($query_p)) {
  ...
  //$status_date = $row_p['status_date'];
    ...
    $sql = "UPDATE task SET status = 'On Going', status_date = NOW() WHERE company_id = '$log_username' AND user_id = '$user_ids' AND task_id=" . $id;
    mysqli_query($db_conx, $sql);
  ...
  $sql = "SELECT * FROM task WHERE project_id = '$pid' AND company_id = '$log_username' AND user_id = '$user_ids'";
