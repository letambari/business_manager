<?php include 'parsers/header.php'; ?>
            ...
            <div class="page-wrapper">
                ...
                <div class="content container-fluid">
					...
					<div class="page-header">
						<div class="row">
							<div class="col-sm-12">
								<h3 class="page-title">Welcome, <?php echo $e_firstnames; ?> <?php echo $e_lastnames; ?></h3>
								<ul class="breadcrumb">
	...
	<?php 
	$totalexpense = '';
    $gettheexpense = "SELECT sum(grand_total)  AS monexpense FROM expense WHERE company_id = '$log_username' AND status = 'Paid'";
...
$crrntdte=date("Y-m-d");
$query3=mysqli_query($db_conx,"select sum(grand_total) as monthlyexpense from expense where ((paymentdate) between '$monthdate' and '$crrntdte') && (company_id = '$log_username') && (status = 'Paid');");
...
$income = '';
$sql_count_invoice = "SELECT sum(grand_total) as income FROM invoice WHERE company_id = '$log_username'";
...
$query=mysqli_query($db_conx,"select sum(grand_total) as lastincome from invoice where ((invoice_date) between '$monthdates' and '$crrntdtes') && (company_id = '$log_username');");
...
$lastincome=$result['lastincome'];
 ...
 $profit = '';
$sql_count_invoicep = "SELECT sum(grand_total) as profit FROM invoice WHERE company_id = '$log_username' AND status = 'Paid'";
...
$querye=mysqli_query($db_conx,"select sum(grand_total) as lastprofit from invoice where ((invoice_date) between '$monthdatese' and '$crrntdtese') && (company_id = '$log_username') && (status = 'Paid');");
...
$queryed=mysqli_query($db_conx,"select count(employee_id) as employee_id from employees where ((join_date) between '$monthdatese' and '$crrntdtese') && (company_id = '$log_username');");
...
$taskcomplete = '';
    ...
    if ($countthetaskCompleted == 0 || $countthetask == 0) {
        ...
        $h = 100;
    	$taskcomplete = $countthetaskCompleted / $countthetask * $h;
    }
$countthetaskhold = '';
$taskhold = '';
